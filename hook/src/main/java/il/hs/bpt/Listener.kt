...
import il.hs.bpt.script.ScriptDbHelper
import il.hs.bpt.script.ScriptDbManager
import il.hs.bpt.script.parseScript
import il.hs.bpt.utils.ERUD_URL
import il.hs.bpt.utils.Log
import il.hs.bpt.utils.XMLHttpRequest
import il.hs.bpt.utils.invalidUserScriptUrls
import il.hs.bpt.utils.isBptFrontEnd
import il.hs.bpt.utils.isDevToolsFrontEnd
import il.hs.bpt.utils.isUserScript
import il.hs.bpt.utils.matching
import il.hs.bpt.utils.parseOrigin
...
object Listener {
  ...
  private fun checkPermisson(action: String, key: Double, tab: Any?): Boolean {
    ...
    if (allowedActions.get("front-end")!!.contains(action) && !isBptFrontEnd(url)) return false
  ...
  fun on(action: String, payload: String = "", currentTab: Any? = null): String? {
    ...
    when (action) {
      ...
      "block" -> {
        ...
        callback = "if (Symbol.Bpt) Symbol.Bpt.lock(${Local.key},'${Local.name}');"
      }
      "focus" -> {
      ...
      "notification" -> {
        ...
        val builder =
            Notification.Builder(ctx, channel)
                ...
                .setContentText(text)
                .setSmallIcon(
                    Icon.createWithResource("il.hs.bpt", R.drawable.ic_extension))
      ...
      "inspectPages" -> {
        if (WebViewHook.isInit) WebView.setWebContentsDebuggingEnabled(true)
        Chrome.IO.submit {
          val code = "Bpt.post('inspect_pages', ${getInspectPages()});"
          Chrome.evaluateJavascript(listOf(code), currentTab)
        }
      }
      "userscript" -> {
        if (payload == "") {
          ...
          callback = "Bpt.post('userscript', ${detail});"
        } else {
          val data = JSONObject(payload)
          if (data.has("meta")) {
            ...
            if (data.optBoolean("delete")) {
              ...
              callback = "Bpt.post('script_meta', ${result});"
            }
          }
        }
      }
      "extension" -> {
        if (payload == "") {
          if (BuildConfig.DEBUG) {
            callback = "Bpt.post('extension', ${LocalFiles.start()});"
          } else {
        ...
        if (detail.has("message")) {
          ...
          fun response(res: JSONObject) {
            if (Chrome.checkTab(currentTab)) {
              Chrome.evaluateJavascript(listOf("Bpt.post('websocket', ${res})"), currentTab)
            } else {
              target?.close()
            }
          }
...
private class ScriptNotification(detail: JSONObject) : BroadcastReceiver() {
  ...
  companion object {
    const val ACTION_USERSCRIPT = "Bpt"
