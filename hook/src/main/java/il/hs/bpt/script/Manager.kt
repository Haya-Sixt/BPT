...
import il.hs.bpt.utils.resolveContentUrl
...
object ScriptDbManager {
  ...
  init {
    ...
    keepStorage =
        ctx.getSharedPreferences("Bpt", Context.MODE_PRIVATE).getBoolean("keep_storage", true)
  }
  ...
  fun invokeScript(url: String, webView: Any? = null) {
    ...
    if (trustedPage) {
      codes.add("globalThis.Bpt = Symbol.Bpt;")
    } else if (runScripts) {
      codes.add("Symbol.Bpt.lock(${Local.key}, '${Local.name}');")
    }
    codes.add("//# sourceURL=local://Bpt/init")
