...
import il.hs.bpt.utils.randomString
  ...
  fun bootstrap(
    ...
    script.grant.forEach {
      when (it) {
        ...
        "GM.Bpt" -> return@forEach
        else ->
            if (localScript.containsKey(it)) {
              grants += localScript.get(it)
            } else if (it.startsWith("GM_")) {
              grants +=
                  "function ${it}(){ console.error('${it} is not implemented in Bpt yet, called with', arguments) }\n"
...
object Local {
  ...
  val anchorInBpt: Int
  // lineNumber of the anchor in GM.js, used to verify Bpt.dispatch
  ...
  init {
    ...
    eruda =
        "eruda._styles = ${css};\n" +
            ctx.assets
                ...
                .bufferedReader()
                .use { it.readText() }
                .replaceFirst("Symbol.Bpt", "Symbol." + name)
                .replaceFirst("BptUnlockKeyForEruda", key.toString())
    ...
    // Use empty lines to randomize anchorInBpt
    val parts =
        localScript[0]
            .replaceFirst("Symbol.Bpt", "Symbol." + name)
            .replaceFirst("BptUnlockKeyForInit", key.toString())
            .split("\n")
            .filter { if (it.length != 0) true else Random.nextDouble() > seed }
    anchorInBpt = parts.indexOfFirst { it.endsWith("// Kotlin anchor") } + 2
    initBpt = parts.joinToString("\n")
    openEruda =
        localScript[1]
            .replaceFirst("Symbol.Bpt", "Symbol." + name)
            .replaceFirst("BptUnlockKeyForEruda", key.toString())
